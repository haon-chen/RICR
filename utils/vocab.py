...
from utils.Constants import PAD_WORD, UNK_WORD, BOS_WORD, EOS_WORD 
...
class Vocab(object):
    ...
    def __init__(self, filename=None, initial_tokens=None, lower=True):
        ...
        self.bos_token = BOS_WORD
        self.eos_token = EOS_WORD
    ...
    def init_pretrained_embeddings(self, embed_dim, pretrained_embedding):
        ...
        for token in [self.bos_token, self.eos_token]:
            self.embeddings[self.get_id(token)] = np.random.rand(self.embed_dim)
        #self.pretrained = {}
        pretrained_count = 0
